namespace Blacklist.Api.Firewall.Windows
{
    public class WindowsFirewall
    {
        ...
        {
            var netshArgs =
                $"advfirewall firewall add rule name=\"{connectionData.RuleName}\" dir=in interface=any action=block enable=yes profile=Any remoteip=\"{connectionData.Ip}\"";
        ...
        {
            var netshArgs =
                $"advfirewall firewall delete rule name=\"{connectionData.RuleName}\" remoteip=\"{connectionData.Ip}\"";
            var result = WindowsCmd.GetCommandOutput("netsh.exe", netshArgs);
            return result; //OK
        }
    }
